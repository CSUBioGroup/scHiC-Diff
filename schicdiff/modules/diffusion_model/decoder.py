...
from schicdiff.modules.layers.scmodel import EmbeddingDict  # FIX: EmbeddingDict will be refactored to CombinedConditioner

# dim: 512
# out_dim: = in_dim，还原回最初的形状 eg. 10731
# num_layers = 1  post_cond_layers
# cond_num_dict: post_cond_num_dict, None
class Decoder(nn.Module):
    def __init__(self, dim, out_dim, dropout=0., norm_type="layernorm", num_layers=1, cond_num_dict=None,
        ...
        super().__init__()
        if isinstance(act, str) or act is None:  # 说明没创建，需要再实例化一下
        ...
        self.layers = nn.ModuleList()  # FIX: use MLP layer  使用多层感知机

        # 在Decoder之前加MLP dim=512
        # 如果num_layer=1，就没有这一层
    ...
    def forward(self, x, conditions=None):
        # print("== Cian Test ==: self.condition Embed:")
        # print(self.cond_embed)
        ...
        for layer in self.layers:
            # print("== Cian Test ==: x进入post layers 第", i, "层")
            # i += 1
            x = layer(x)
        return x
