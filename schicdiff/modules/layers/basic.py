...
from schicdiff.utils.modules import batch_apply_norm, create_activation, create_norm


class FeedForward(nn.Module):
    def __init__(self, dim, dim_out=None, mult=4, glu=False, dropout=0.):
        super().__init__()
        inner_dim = int(dim * mult)   # 512x4=2048
        dim_out = default(dim_out, dim)  # 没显式设置就是512

        project_in = nn.Sequential(
            nn.Linear(dim, inner_dim),   # dim to inner_dim
            nn.GELU()
        ) if not glu else GEGLU(dim, inner_dim)

        self.net = nn.Sequential(
            project_in,
            nn.Dropout(dropout),  # dropout的概率是0，就不起作用
            nn.Linear(inner_dim, dim_out)  # inner_dim to dim_out(dim)
        )
