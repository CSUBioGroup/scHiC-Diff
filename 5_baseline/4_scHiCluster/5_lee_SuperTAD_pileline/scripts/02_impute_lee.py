#!/usr/bin/env python3
"""Run scHiCluster imputation for Lee SuperTAD pipeline data.
...
Data geometry:
  chrom       = chr4
  n_bins      = 49
  resolution  = 10000  (passed as --res 10000)
  chrom.sizes = chr4\t48  (scHiCluster internally creates 49 bins)

Output format: npz (sparse CSR, 49x49 per cell)
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run hicluster impute-cell for Lee SuperTAD pipeline."
    )
    parser.add_argument("--dataset", required=True,
                        help="Cell type directory name (e.g. Astro)")
    ...
    parser.add_argument("--window-size", type=int, default=500000,
                        help="Window size in bp (must be >= n_bins * res)")
    parser.add_argument("--step-size", type=int, default=500000,
                        help="Step size in bp")
...
_W_CHROM: str = CHROM
_W_RES: int = RESOLUTION
_W_CHROM_FILE: Path = Path()
_W_PAD: int = 1
_W_STD: float = 1.0
_W_RP: float = 0.5
_W_TOL: float = 0.01
_W_WINDOW: int = 500000
_W_STEP: int = 500000
_W_OUT_DIST: int = 480000  # (N_BINS - 1) * RESOLUTION
_W_OUT_FMT: str = "npz"
...
def main() -> int:
    ...
    init_kwargs = {
        ...
        "output_dist": (args.n_bins - 1) * args.resolution,
    ...
    print(f"[impute] {args.dataset}: total={total_elapsed:.1f}s "
          ...
          f"avg_cell={sum(cell_times)/max(len(cell_times),1):.2f}s, "
