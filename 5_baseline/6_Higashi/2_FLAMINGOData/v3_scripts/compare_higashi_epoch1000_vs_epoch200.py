#!/usr/bin/env python3
"""Compare FLAMINGO v3 Higashi updates=1000 subset against updates=200 baseline."""
...
TASKS = (
    ("v3_hybrid_W0p5_500cells_level0", 0),
    ("v3_hybrid_W0p5_500cells_level0", 5),
    ("v3_hybrid_W0p7_500cells_level0", 0),
    ("v3_hybrid_W0p7_500cells_level0", 5),
)
...
def aggregate_log1p_task_metrics(experiment_root: Path, metrics_dir: Path) -> pd.DataFrame:
    ...
    out = metrics_dir / "higashi_epoch1000_subset_log1p_metrics.csv"
    df.to_csv(out, index=False)
    print(f"[compare] saved {out} ({len(df)} rows)", flush=True)
    return df
...
def main() -> int:
    ...
    for stem, neighbor in TASKS:
        print(f"[compare] {stem} nbr{neighbor}", flush=True)
    ...
    raw_out = metrics_dir / "higashi_epoch1000_subset_raw_gt_PCC_MAE.csv"
    raw_df.to_csv(raw_out, index=False)
    print(f"[compare] saved {raw_out} ({len(raw_df)} rows)", flush=True)

    summary = pd.DataFrame(summary_rows)
    summary_out = metrics_dir / "higashi_epoch1000_vs_epoch200_summary.csv"
    summary.to_csv(summary_out, index=False)
    print(f"[compare] saved {summary_out} ({len(summary)} rows)", flush=True)
    show_cols = [
        "dataset",
        "neighbor",
