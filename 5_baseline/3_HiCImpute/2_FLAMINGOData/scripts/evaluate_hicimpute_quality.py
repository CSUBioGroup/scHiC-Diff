#!/usr/bin/env python3
"""Evaluate HiCImpute FLAMINGO imputation quality."""
...
DEFAULT_MANIFEST = BASE_DIR / "input" / "manifest.tsv"
...
def parse_dataset_id(dataset_id: str) -> dict[str, object]:
    ...
    if not match:
        return {"beads": np.nan, "w": "", "level": np.nan, "timepoint": ""}
...
def evaluate_cell(pred: np.ndarray, truth: np.ndarray, observed: np.ndarray) -> dict[str, float]:
    ...
    for prefix in ("all", "observed", "heldout"):
...
def summarize_cells(cell_df: pd.DataFrame) -> dict[str, float]:
    ...
    for col in cell_df.columns:
        if col in {"dataset_id", "mode", "cell_idx", "cell_name"}:
            continue
...
def evaluate_dataset(record: DatasetRecord, output_root: Path, mode: str) -> tuple[pd.DataFrame, dict[str, object]]:
    pred_path = output_root / "npz_lower_tri" / f"{record.dataset_id}_hicimpute_{mode}_lower_tri.npz"
...
def summarize_overall(summary_df: pd.DataFrame) -> pd.DataFrame:
    ...
    numeric_cols = [
        ...
        if col not in {"dataset_id", "mode", "timepoint"}
        and pd.api.types.is_numeric_dtype(summary_df[col])
    ]
    rows = []
    for mode, group in summary_df.groupby(group_cols, dropna=False):
        if isinstance(mode, tuple):
            mode_value = mode[0]
        else:
            mode_value = mode
        row: dict[str, object] = {"mode": mode_value, "n_datasets": int(len(group))}
        for col in numeric_cols:
            row[f"{col}_mean_across_datasets"] = float(group[col].mean(skipna=True))
            row[f"{col}_std_across_datasets"] = float(group[col].std(skipna=True))
        rows.append(row)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--modes",
        nargs="+",
        choices=["Impute_All", "Impute_SZ"],
        default=["Impute_All", "Impute_SZ"],
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    for mode in args.modes:
        for record in records:
            print(f"Evaluating {record.dataset_id} {mode}", flush=True)
            cell_df, summary = evaluate_dataset(record, args.output_root, mode)
    ...
    cell_path = args.metrics_dir / "hicimpute_flamingo_cell_metrics.csv"
    dataset_path = args.metrics_dir / "hicimpute_flamingo_dataset_summary.csv"
    overall_path = args.metrics_dir / "hicimpute_flamingo_overall_summary.csv"
