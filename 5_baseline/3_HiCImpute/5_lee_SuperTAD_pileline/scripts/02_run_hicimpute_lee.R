...
# HiCImpute MCMCImpute for Lee SuperTAD pipeline data (chr4, 49 bins, 1176 features)
# Adapted from v3_run_hicimpute_hicimputedata.R
...
main <- function() {
  ...
  dataset <- argval(args, "--dataset")       # cell type name, e.g. "Astro"
  input_root <- argval(args, "--input-root")
  output_root <- argval(args, "--output-root")
  n_beads <- as.integer(argval(args, "--n-beads", "49"))
  ...
  dir.create(file.path(output_root, "npz_triu"), recursive = TRUE, showWarnings = FALSE)
  ...
  # Restore to numpy row-major triu order
  impute_all_r <- result$Impute_All
  impute_all_numpy <- impute_all_r[inv_order, , drop = FALSE]
  pred_cells_by_features <- t(impute_all_numpy)

  npz_path <- file.path(output_root, "npz_triu", paste0(dataset, "_hicimpute_Impute_All_triu.npz"))
