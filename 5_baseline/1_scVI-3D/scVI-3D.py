...
def normalize(bandM, cellInfo, chromSelect, bandDist, nLatent = 100, batchFlag = False, gpuFlag = False):
    # 打印传入的 bandM 的形状
    print(f"Initial bandM shape: {bandM.shape}")

    # cellSelect = [i for i, val in enumerate(bandM.sum(axis = 1)>0) if val]

    # 不过滤
    cellSelect = [i for i,val in enumerate(bandM)]
    print("Cian Test: cellSelect=", len(cellSelect))
    ...
    if len(cellSelect) == 0:
        ...
        print("Cian Test: adata.shape:", adata.shape)
