...
本目录生成修订后的模拟数据图。正文图只包含 HiCImputeData 和 FLAMINGOData
掉落位置的 PCC 与 MAE。结构零（SZ）与掉落（DO）结果仅作为 HiCImputeData 的
补充诊断保留，因为真实 scHi-C 数据没有可观测的 SZ/DO 标签，而 FLAMINGOData
不存在 `GT == 0` 类别。

所有命令均从 `paperplots/3_dropoutBar_confuseMap/` 运行。

## 评价范围

连续值诊断与分类诊断共享 GT、观测矩阵、插补输出、细胞、特征顺序和原始 contact
尺度，但回答不同的问题：

```text
分类全集   = observed == 0
留出 DO    = observed == 0 and GT > 0
真实 SZ    = observed == 0 and GT == 0

留出 DO 是分类全集的严格子集
```

- 正文 PCC/MAE 柱状图只在真实 DO（即留出的正 contact）上评价连续值恢复。
- F1、混淆矩阵、ROC 和 PR 曲线只作为补充材料，在所有 observed-zero
  HiCImputeData contact 上进行 SZ/DO 诊断。
...
`calculate_sz_do_crossfit_metrics.py` 根据当前正式插补矩阵重新计算 HiCImputeData
的 SZ/DO 工作点指标。方案固定如下：
...
scHiC-Diff 本身输出连续插补 contact 值，并没有结构零分类器。规则
`prediction < threshold => predicted SZ` 属于后处理诊断。阈值随方法和深度而异，
不是 0.5 之类的通用值。

`calculate_sz_do_roc_pr.py` 针对每种方法、细胞类型和深度，扫描合并后的五折留出
得分分区中每个不同的插补值。脚本写出精确阈值扫描、ROC-AUC 和 PR-AUC（平均精确率）。
排序得分为 `-imputed_contact`，因此不同方法的单调输出尺度不会影响 ROC/PR 比较。
...
HiCImputeData 补充 SZ/DO 工作点指标、完整 ROC/PR 扫描和配套图：
...
`plot_main_dropout_figure.py` 在
`figures/main_dropout_figure/Simulated_dropout_PCC_MAE_main_figure.pdf`
生成正文图。Panel A 为 HiCImputeData，Panel B 为 FLAMINGOData；两个面板均只展示
掉落位置的 PCC 和 MAE。该图由 Matplotlib 原生渲染，共用七种方法的图例，不包含
SZ/DO 分类面板。

> **图 X | 两个模拟 scHi-C 基准上的掉落恢复。** PCC（越高越好）和 MAE（越低越好）
> 仅在原始 contact 尺度的真实掉落 contact（`observed = 0` 且 `GT > 0`）上计算。
> Panel A 展示不同细胞类型和测序深度的 HiCImputeData；Panel B 展示 W 和 P 模拟扫描的
> FLAMINGOData。柱和误差条表示逐细胞均值和标准差。

SZ/DO 图，包括固定 MCC 工作点折线、混淆矩阵、完整 ROC/PR 曲线和 ROC-AUC/AP
热图，应放入补充材料。推荐的方法描述和审稿回复措辞见
`STRUCTURAL_ZERO_REVISION.md`。
