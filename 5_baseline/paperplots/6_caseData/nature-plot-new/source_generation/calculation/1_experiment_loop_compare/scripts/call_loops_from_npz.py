...
def flat_to_matrix(flat, n_bins):
...
def process_single_cell(flat_vec, n_bins, resolution, max_dist, cap, pad, gap):
    contact = flat_to_matrix(flat_vec, n_bins)
...
def merge_cells(data_matrix, n_bins, resolution, max_dist, cap, pad, gap):
...
def run_case(
    ...
    n_bins = n_bins or infer_n_bins(n_features)
