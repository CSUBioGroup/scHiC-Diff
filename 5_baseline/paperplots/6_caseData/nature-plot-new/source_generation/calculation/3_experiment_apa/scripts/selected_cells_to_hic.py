...
def infer_n_bins(n_features):
    n_bins = int((-1 + np.sqrt(1 + 8 * n_features)) / 2)
    if n_bins * (n_bins + 1) // 2 != n_features:
        raise ValueError(f"Cannot infer n_bins from n_features={n_features}")
    return n_bins


def triu_to_matrix(triu_flat, n_bins):
...
def npz_to_pre(npz_file, pre_file, resolution=20000, chrom="chr1"):
    ...
    n_bins = infer_n_bins(n_features)

    merged = np.zeros((n_bins, n_bins))
    for cell_idx in range(n_cells):
        merged += triu_to_matrix(mat[cell_idx].toarray().ravel(), n_bins)
...
def convert_single(npz_file, hic_file, juicer_jar, resolution=20000, chrom="chr1", chrom_size=None, keep_pre=False):
    hic_file = Path(hic_file)
    hic_file.parent.mkdir(parents=True, exist_ok=True)
    pre_file = hic_file.with_suffix(".pre")
    n_bins = npz_to_pre(npz_file, pre_file, resolution=resolution, chrom=chrom)
...
def convert_directory(samples_dir, hic_dir, juicer_jar, resolution=20000, chrom="chr1", chrom_size=None, keep_pre=False):
