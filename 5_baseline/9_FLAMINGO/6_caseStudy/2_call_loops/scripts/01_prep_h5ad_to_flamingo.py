#!/usr/bin/env python3
"""Prepare 20kb chr1 case-study h5ad data for Tensor-FLAMINGO contact-space imputation.

For the given chromosome h5ad:
...
DEFAULT_INPUT_ROOT = Path(
    "/public/home/hpc254701055/2_projects/10_schicdiff/1_scHiC/5_baseline/"
    "paperplots/6_caseData/2_callLoop_apa/inputData/6_hires_chr1"
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--resolution", type=int, default=20000,
                   help="Resolution in bp (used only for metadata)")
    p.add_argument("--chroms", nargs="*", default=None,
                   help="Subset of chromosomes (default: chr1)")
...
def prepare_chrom(chrom: str, args: argparse.Namespace, resolution: int) -> Path:
    input_path = args.input_root / f"hires_{resolution // 1000}kb_{chrom}.h5ad"
...
def main() -> int:
    args = parse_args()
    chroms = args.chroms or [c for c in DEFAULT_CHROMS
                             if (args.input_root / f"hires_{args.resolution // 1000}kb_{c}.h5ad").exists()]
    print(f"[prep] chromosomes: {chroms}", flush=True)
    for chrom in chroms:
        prepare_chrom(chrom, args, args.resolution)
