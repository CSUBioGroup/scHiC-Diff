...
import resource
import platform
from omegaconf import OmegaConf  # 确保 OmegaConf 被导入

import anndata  # <--- 修改点 1: 新增 import
import numpy as np
...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument("--save_path", type=str, default="/home/duxuyan/Projects/DiffusionModel/scHiC-Diff-master/results",
                        help="path to save the imputed data of test results ")
    ...
    parser.add_argument("--wandb", type=str2bool, const=True, default=False, nargs="?", help="enable wandb logger")
    ...
    parser.add_argument("--ckpt_every_n_epochs", type=int, default=500)
    parser.add_argument("-cg", "--custom_clip_gradient", type=float, default=None)
    parser.add_argument("-om", "--overwrite_model", type=str2bool, const=True, default=True, nargs="?",
...
# === 修改点 2: 修改 DataModuleFromConfig 类 ===
...
class DataModuleFromConfig(pl.LightningDataModule):
    def __init__(self, batch_size, train=None, validation=None, test=None, predict=None,
        ...
        self.preloaded_adata = preloaded_adata
    ...
    def _setup(self):
        self.datasets = {}
        print("Dataset setup....... ")
        if self.preloaded_adata is None:
            print("Warning: No preloaded data found, falling back to inefficient per-dataset loading.")
            ...
            print("Using preloaded anndata object to set up datasets.")
            for data_name, data_cfg in self.dataset_configs.items():
                print("datset:", data_name)
                
                # === 核心修改点 ===
                # 1. 使用字典方式访问和修改
                data_cfg['params']['adata'] = self.preloaded_adata
                
                # 2. 【已修正】我们不再将 data_cfg 转回 OmegaConf 对象。
                #    直接将这个包含了 AnnData 对象的纯 Python 字典传递给实例化函数。
                data_obj = instantiate_from_config(data_cfg)
                # =================

                self.datasets[data_name] = WrappedDataset(data_obj) if self.wrap else data_obj
                print("datset:", data_name, "初始化完毕")
                print("cond_num_dict: ", data_obj.cond_num_dict)
                print("post_cond_num_dict: ", data_obj.post_cond_num_dict)
                print()
                print()
...
# ====> 在这里或 CUDACallback 附近，添加下面的新类 <====
class CPUMemoryCallback(Callback):
    """
    一个用于监控和记录 CPU 内存使用情况的回调函数。
    它会在每个训练周期结束时，记录到目前为止的峰值内存占用。
    """
    def on_train_epoch_end(self, trainer, pl_module):
        # resource.getrusage 只能在 Unix-like 系统 (如 Linux) 上工作
        if platform.system() == "Linux":
            # ru_maxrss 返回的是“最大常驻集大小”，即进程所占用的峰值物理内存（RAM）
            # 单位是 Kilobytes (KB)
            peak_mem_kb = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
            # 将 KB 转换为 GB 以方便阅读
            peak_mem_gb = peak_mem_kb / (1024 * 1024)
            
            # 使用 rank_zero_info 确保在多进程环境中只打印一次
            rank_zero_info(f"Peak CPU Memory Usage up to this epoch: {peak_mem_gb:.2f} GB")
            
            # 使用 self.log 将指标记录到日志文件 (如 CSV, Wandb) 中
            pl_module.log("performance/peak_cpu_mem_gb", peak_mem_gb, prog_bar=True)
...
class CUDACallback(Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module, outputs=None):
        max_memory = 0.0  # 为 CPU 模式设置一个默认值
        # === 核心修改点 ===
        # 同样，只在 CUDA (GPU) 可用时才获取显存信息
        if torch.cuda.is_available():
            torch.cuda.synchronize(trainer.strategy.root_device.index)
            max_memory = torch.cuda.max_memory_allocated(trainer.strategy.root_device.index) / 2 ** 20
        
        epoch_time = time.time() - self.start_time
        print("epoch spends time:", epoch_time)
...
def maybe_hack_get_best_ckpt_path(ckpt_path, load_best):
    ...
    if ckpt_path is None:
        for cbk in trainer.callbacks:
            ...
            warnings.warn("Unable to retrieve last checkpoint path, falling back to using default checkpoint",
                          RuntimeWarning, stacklevel=2)
            return None
    cbks = torch.load(ckpt_path).get("callbacks")
    if cbks is None:
        warnings.warn(f"Unable to retrieve callbacks info (contains checkpoints info) from {ckpt_path}, "
                      "falling back to using default checkpoint.", RuntimeWarning, stacklevel=2)
        return None
...
if __name__ == "__main__":
    ...
    # 只有在 CUDA 可用时，才打印详细的 GPU 信息
    if cuda_available:
        print("GPU count:", torch.cuda.device_count())
        print("Current device:", torch.cuda.current_device())
        print("Device name:", torch.cuda.get_device_name(0))
    else:
        print("Running on CPU mode, no GPU details to report.")
    # ==========================
    ...
    try:
        ...
        trainer_config = OmegaConf.to_container(lightning_config.get("trainer", OmegaConf.create()), resolve=True)
        ...
        if "devices" not in trainer_config:
            ...
            if isinstance(gpuinfo, list) and len(gpuinfo) > 1:
        ...
        print("#### Data #####")
        if not opt.train:
            assert ('test' in config.data.params.keys() or 'predict' in config.data.params.keys())
            config.data.params.pop('train', None)
            config.data.params.pop('validation', None)
        if opt.no_test:
            config.data.params.pop('test', None)
        
        # === 修改点 3: 在实例化 data 对象之前，加入我们的新逻辑 ===
       # ... (前面的代码) ...
        # ===> 新增：只加载一次数据
        print("#### Pre-loading data once to save memory ####")
        # 从训练配置中获取数据文件名（绝对路径）
        data_filepath = config.data.params.train.params.fname
        print(f"Loading data from: {data_filepath}")
        # 使用 anndata 读取文件
        full_adata = anndata.read_h5ad(data_filepath)
        print("Data loaded successfully into memory.")

        # === 核心修改点 ===
        # 1. 将 OmegaConf 配置转换为普通的 Python 字典
        data_params = OmegaConf.to_container(config.data.params, resolve=True)
        # 2. 将预加载的数据对象添加到这个普通字典中
        data_params['preloaded_adata'] = full_adata
        
        print("#### Data #####")
        # 3. 直接使用这个包含所有参数的字典来创建 DataModuleFromConfig 实例，
        #    从而绕过 OmegaConf 的限制。
        data = DataModuleFromConfig(**data_params)
        data._setup()
        # =================================================================
        ...
        if ('pretrained_gene_list' not in config.model.params.model_config.params.keys()
        ...
        assert opt.save_path
        config.model.params.save_path = opt.save_path
        config.model.params.model_config.params.save_path = opt.save_path
        print("Save result in ", config.model.params.save_path)
        ...
        default_logger_cfgs = {
            "wandb": {"target": "pytorch_lightning.loggers.WandbLogger",
                      "params": {"project": opt.wandb_project, "entity": "ligroup", "group": opt.group,
                                 "name": nowname, "save_dir": logdir, "offline": opt.wandb_offline, "id": nowname}},
            "csv": {"target": "pytorch_lightning.loggers.CSVLogger",
                    "params": {"name": "csv", "save_dir": logdir}}}
        ...
        default_callbacks_cfg = {
            "setup_callback": {"target": "main.SetupCallback",
                               "params": {"resume": opt.resume, "now": now, "logdir": logdir, "ckptdir": ckptdir,
                                          "cfgdir": cfgdir, "config": config, "lightning_config": lightning_config}},
            "learning_rate_logger": {"target": "pytorch_lightning.callbacks.LearningRateMonitor",
                                     "params": {"logging_interval": "step"}},
            "cuda_callback": {"target": "main.CUDACallback"},
            "cpu_memory_callback": {
                "target": "main.CPUMemoryCallback"},
            ...
            default_metrics_over_trainsteps_ckpt_dict = {
                'metrics_over_trainsteps_checkpoint':
                    {"target": 'pytorch_lightning.callbacks.ModelCheckpoint',
                     'params': {"dirpath": os.path.join(ckptdir, 'trainstep_checkpoints'),
                                "filename": "{epoch:06}-{step:09}", "verbose": True, 'save_top_k': -1,
        ...
        trainer_kwargs.update(dict(trainer_config))
        trainer = pl.Trainer(**trainer_kwargs)
        print("Trainer initialization successful")
        trainer.logdir = logdir
        print("log dir:", trainer.logger.save_dir)

        bs, base_lr = config.data.params.batch_size, config.model.base_learning_rate
        ngpu = len(lightning_config.trainer.devices) if not cpu else 1
        print("Cian Test: ngpu =", ngpu)
        accumulate_grad_batches = lightning_config.trainer.get("accumulate_grad_batches", 1)
        print("=== Cian Test === 积累几个batch才会更新参数:")
        ...
        def check_weights(model, ckpt_path, load_conditions=True):
            ...
            loadable_state_dict = {k: v for k, v in ckpt_state_dict.items() if k in model_dict and v.shape == model_dict[k].shape}
            ...
            if len(new_genes) > 0:
                ...
                pretrained_embed = {encoder_embed_key: ckpt_state_dict[encoder_embed_key],
            ...
            if opt.train:
                if ckpt_path is not None:
                    ...
                    if len(ignored_keys) > 0 or len(not_pretrained_keys) > 0 or opt.weights_only:
                        ...
                        model.unique_conditions = None
                        trainer.fit(model, data)
                    else:
                        print("========================= trainer.train ===============================")
                        trainer.fit(model, data, ckpt_path=ckpt_path)
                else:
                    print("============ckpt none============= trainer.train ===============================")
                    trainer.fit(model, data)
