...
#SBATCH --job-name=hires_proprecess
#SBATCH --output=./tools/hires_proprecess_job_%j.log
#SBATCH --error=./tools/hires_proprecess_job_%j.log
...
cd "$PROJECT_ROOT"
echo "Changed directory to project home: $PROJECT_ROOT"
...
# 输入输出路径
INPUT_DIR="${PROJECT_ROOT}/data/h5ad_final_10000"
OUTPUT_DIR="${PROJECT_ROOT}/data/h5ad_final_10000/preprocessed"
...
process_file() {
    ...
    # 执行预处理
    if ${PYTHON_EXEC} tools/kb_offline_preprocess.py \
        --input "${INPUT_FILE}" \
        --output "${OUTPUT_FILE}" \
