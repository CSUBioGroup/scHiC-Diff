...
# ==============================================================================
#SBATCH --job-name=fatq_100k_hires_impute       # 作业名称
#SBATCH --output=./run/logs/fatq_hires_100k_imputejob_%j.log          # 标准输出日志
#SBATCH --error=./run/logs/fatq_hires_100k_imputejob_%j.log           # 标准错误日志 (与输出合并，强烈推荐)
#SBATCH --account=pi_limin_r                    # 您的账户
#SBATCH --partition=fatQ                 # 计算分区/队列
#SBATCH --mem=500G 
#SBATCH --qos=fatq                        # 服务质量
#SBATCH --nodes=1                         # 节点数
#SBATCH --ntasks-per-node=1               # 每节点任务数
#SBATCH --cpus-per-task=40                # CPU 核心数 (请根据数据加载等需求调整)
...
# --- 准备数据参数 (使用绝对路径，确保万无一失) ---
    ...
    # 构建文件的【绝对路径】，这是最稳健的方法
    # HOMEDIR 变量已经在脚本上部被定义为项目的绝对路径
    # absolute_data_fname="${HOMEDIR}/${data_subdir}/${base_fname}"
    absolute_data_fname="/public/home/hpc254701055/Projects/1_scHiCDiff/3_DiffusionModel/scHiC-Diff-master/data/2_hires_schicdiff/h5ad_final_100000/${base_fname}"
