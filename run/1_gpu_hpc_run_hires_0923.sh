...
# ==============================================================================
#SBATCH --job-name=gpu_100k_hires_impute       # 作业名称
#SBATCH --output=./run/logs/gpu_hires_100k_imputejob_%j.log          # 标准输出日志
#SBATCH --error=./run/logs/gpu_hires_100k_imputejob_%j.log           # 标准错误日志 (与输出合并，强烈推荐)
#SBATCH --account=pi_limin_r                    # 您的账户
#SBATCH --partition=gpu4Q                 # 计算分区/队列
#SBATCH --mem=340G 
#SBATCH --qos=gpuq                        # 服务质量
#SBATCH --nodes=1                         # 节点数
#SBATCH --ntasks-per-node=1               # 每节点任务数
#SBATCH --cpus-per-task=20                # CPU 核心数 (请根据数据加载等需求调整)
...
# --- 准备数据参数 (使用绝对路径，确保万无一失) ---
    ...
    absolute_data_fname="/public/home/hpc254701055/Projects/1_scHiCDiff/3_DiffusionModel/scHiC-Diff-master/data/2_hires_schicdiff/h5ad_final_1000000/${base_fname}"
